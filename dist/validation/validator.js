export function validateSkillContent(content, metadata) {
    const errors = [];
    const warnings = [];
    ...
    if (curlBash.test(content))
        errors.push("Suspicious install command: curl pipe to shell");
    if (wgetSh.test(content))
        errors.push("Suspicious install command: wget pipe to shell");
    if (content.includes('`'))
        errors.push("Suspicious backtick execution detected");
    if (/\$\(/.test(content))
        errors.push("Suspicious command substitution detected");
    // 3. Path traversal
    ...
    if (content.includes('eval('))
        warnings.push("Contains eval() — possible code execution");
    if (content.includes('exec('))
        warnings.push("Contains exec() — possible code execution");
