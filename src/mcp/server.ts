...
async function handleSearchSkills(params: Record<string, unknown>): Promise<string> {
...
async function handleRecommendSkills(params: Record<string, unknown>): Promise<string> {
...
async function handleSkillInfo(params: Record<string, unknown>): Promise<string> {
...
async function handleInstallSkill(params: Record<string, unknown>): Promise<string> {
  const identifier = params.identifier;
  if (typeof identifier !== "string" || identifier.trim().length === 0) {
    throw new Error("identifier is required and must be non-empty");
  }
...
export class MCPServer {
   ...
   */
  async processLine(line: string): Promise<JsonRpcResponse | null> {
    try {
