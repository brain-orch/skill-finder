...
interface AwesomeSkillSearchResponse {
  query: string;
  count: number;
  skills: AwesomeSkillItem[];
}

export class AwesomeSkillMarketplace implements SkillMarketplace {
  name = "awesomeskill" as const;
  ...
  async search(
    ...
    try {
      ...
      const json: AwesomeSkillSearchResponse =
        await response.json() as AwesomeSkillSearchResponse;

      if (!json.skills || !Array.isArray(json.skills)) return [];

      const results: SkillSearchResult[] = json.skills.map((skill) => ({
        id: `awesomeskill:${skill.slug}`,
        name: skill.name,
        description: skill.description,
        marketplace: "awesomeskill" as const,
  ...
  async install(
    ...
    const repoMatch = info.installCommand.match(
