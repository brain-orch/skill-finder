...
export class TrustScorer {
  ...
  score(skill: SkillSearchResult, securityScore?: number): TrustGrade {
    ...
    let grade: TrustGrade["grade"];
    let label: string;

    if (totalScore >= 90) { grade = "A"; label = "Trusted"; }
    else if (totalScore >= 75) { grade = "B"; label = "Reliable"; }
    else if (totalScore >= 60) { grade = "C"; label = "Caution"; }
    else if (totalScore >= 40) { grade = "D"; label = "Review Required"; }
    else { grade = "F"; label = "Review Required"; }
